...
- **Deduction formation is oracle-driven + board-owned** (Phase 2b, enacts [ADR-0012](docs/DECISIONS/ADR-0012-deduction-roll-semantics.md)) — `DeductionButton` **only rolls** the Reason d20 and reports the raw tier via `onResult(tier: OutcomeTier)`; it forms nothing and writes no clue status, and carries no sticky terminal phase (a fresh connection set always re-enables it). `EvidenceBoard.handleDeductionAttempt(tier)` runs the pure `deductionOracle.classifyBoard(connections, clues, recipes)`, then forms **every** qualifying deduction (all matched recipes per component + generic `correct`/`false` sets) via `addDeduction` + `markCluesDeduced`, and marks `partial`/`incorrect` components `contested` via `contestClues`. **Correctness gates formation; the roll only flavours the banner copy** of a `correct` result (a `critical` reads sharper) — a correct set forms even on a `failure` roll, a wrong set forms nothing even on a `critical`. The board owns the transient `aria-hidden` tone-coloured banner (`correctnessToBanner`) + the single `announce()` (`src/announcer.ts`); the banner lives on the board because the button returns `null` below 2 connected clues. The `'connected'` clue status is **derived, not stored** — `ClueCard isConnected` renders the ring + 🔗 from `connections` membership. **The old latent `contested`-revert bug is fixed:** the revert is store-owned + token-gated (`evidenceSlice` `contestedTokens`/`attemptSeq`; see architecture.md), so it survives board unmount and can't clobber across overlapping attempts.
...
- **`Date.now()` and `Math.random()` used directly** — In `diceEngine.rollD20()`, `hintEngine`, `metaSlice.saveGame` (save ID). Not injectable. Tests work around this. (`buildDeduction` no longer uses either — Phase 2b made its generic id a pure sorted-clue signature `deduction-generic-<ids>`. `saveManager` uses neither; it stamps a save's `timestamp` with `new Date().toISOString()`.)
