...
- `deploy.yml` — on push to main, PR to main, or manual dispatch. Three jobs:
  `test` (validator `node scripts/validateCase.mjs` + `npm run test:run`) → `build`
  (`npm audit --audit-level=high` + `npm run build`) → `deploy`. `build` needs `test`,
  so a failing test/validator blocks the merge gate. `deploy` is skipped on PR events
  (`if: github.event_name != 'pull_request'`). `concurrency: cancel-in-progress: false`.
...
the live test baseline — 384 tests across 30 files pass as of 2026-07-07.)

### Critical (blocks core functionality)
- No P0 *release-blockers of the original build*; `loadGame` caseData restoration was fixed in Phase A.
  The 2026-07 repo audit did surface open engine bugs — see the P0 backlog in `docs/PROJECT_STATE.md`.
