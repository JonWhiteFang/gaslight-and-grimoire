...
- `loadVignette(vignetteId: string): Promise<VignetteData>` — same shape for a two-act vignette (single `scenes.json`), also injecting the shared scenes. Also fetches the optional `deductions.json` and `variants.json` (each `.catch`-guarded — most vignettes ship neither) into `VignetteData.recipes?` / `VignetteData.variants?`. **Deliberate asymmetry vs `loadCase`:** recipes fall back to `undefined` (field absent, matching the optional `VignetteData.recipes?` shape) while variants fall back to `[]`; `vignetteToCaseData` passes both through, so a vignette with authored recipes/variants behaves exactly like a main case at runtime.
...
- `validateBundle(bundle: ContentBundle, options?: { includeReachability?: boolean }): { errors: string[]; warnings: string[] }` — errors: choice-outcome edges (base/variant/shared ids valid), `requiresClue`/`advantageIf`/`cluesAvailable`/`onEnter` discoverClue clue refs, `onEnter` npc refs, `npcEffect.npcId` (incl. inside encounter rounds and `worseAlternative`), condition targets (clue/npc/faculty/archetype/faction/suspicion-tier allowlists; `hasFlag value:false` allowed), variant `variantOf`+`variantCondition` presence, encounter-round edge + tier recursion, tier completeness (fixed **or** `dynamicDifficulty`), `clue.sceneSource`, **key-deduction recipes** (`KeyDeduction.requiredClues` clue refs, plus `choice.requiresDeduction` and `hasDeduction`-condition targets against the recipe-id registry — a gate pointing at an undefined recipe is an error; each recipe's optional `onForm` effect list is validated like any other effect list — unknown `discoverClue`/npc targets are errors), and **choice-gating rules** (Phase 5): `disabled` without a non-empty `gateReason`; `gateReason` without `disabled`; `disabled`/`shown` on a choice with no `requires*` gate (explicit `hidden` is an allowed no-op); a `visibility` value outside `shown | hidden | disabled`; and an `isEscapePath` choice setting `visibility: "disabled"`/`"shown"` or a `gateReason` (an explicit `"hidden"` is an allowed no-op). Warnings are no longer reachability-only — two choice-gating warnings are **always on**, independent of `includeReachability`: the **soft-gate warning** (`visibility: "shown"` on a gated choice) and the **soft-lock warning** (a non-empty scene choice list or an encounter round with no *guaranteed-selectable* choice — ungated non-escape, soft-gated `"shown"`, or ungated escape path — including a supernatural round 1 whose `worseAlternative` replacement after a failed reaction check would leave none; empty scene choice lists are terminal scenes and exempt, empty rounds warn). With `includeReachability`, additionally warns on scenes unreachable from `firstScene` and clues no reachable scene can discover — `computeDiscoverableClues` counts **both** a reachable scene's `cluesAvailable` and its `onEnter` `discoverClue` effects as sources (parity with `computeObtainableClues`), so an effect-granted clue no longer false-positives this warning — and **errors** when a clue required by a *gated* key deduction (referenced by `requiresDeduction`/`hasDeduction`/`ClueDiscovery.requiresDeduction`) is only obtainable on a `critical`-tier scene edge — i.e. the gated content would be reachable only by a lucky roll (F-102).
...
- **`KeyDeduction.onForm?: Effect[]`** — optional effects applied **exactly once, at first formation**: `EvidenceBoard.handleDeductionAttempt` calls `worldSlice.applyEffects(recipe.onForm)` only when the recipe id is *not* already in the store's `deductions` record (the deductions record **is** the once-guard — no separate flag). Because deductions round-trip through save/load, the guard survives reload; re-forming the same recipe (e.g. after reconnecting the same clues) never re-fires it. `onForm` is validated by `validateBundle` like any other effect list.
