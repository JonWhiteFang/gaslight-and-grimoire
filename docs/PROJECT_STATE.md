...
- **Branch focus:** `feat/phase5-choice-gating` (branched off `8e1c2b0`) — **Phase 5 IMPLEMENTED, PR
  [#86](https://github.com/JonWhiteFang/gaslight-and-grimoire/pull/86) open** (merge commit, never squash).
  9 tasks + all review folds landed; gate green (816/82, lint, validator 8/8 zero warnings, build); live
  browser verify passed; full Codex spec/plan/impl trail folded. **Next:** merge PR #86 — that completes
  the entire UI/UX roadmap. Prior branches merged: #85 (Phase 4), #84 (Phase 3), #83 (Phase 2b), #82
  (Phase 2a), #76 (Comet Club), #79/#80 (UI/UX Phase 0/1). All audit backlogs cleared except #20 (media,
  user-blocked).
...
**1. Merge PR [#86](https://github.com/JonWhiteFang/gaslight-and-grimoire/pull/86) (Phase 5 — choice-gating)**
via merge commit (never squash) once CI is green. That completes the **entire UI/UX roadmap (Phases 0–5)**.

**2. Content resumes: The Orrery Room** (Grey Dawn vignette — fills the one missing faction vignette +
