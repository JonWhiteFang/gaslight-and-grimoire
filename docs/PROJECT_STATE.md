...
- **Branch focus:** `feat/phase3-dice-legibility` (branched at `087a236`) — **Phase 3 IMPLEMENTED, PR
  [#84](https://github.com/JonWhiteFang/gaslight-and-grimoire/pull/84) open** (merge commit, never squash). 8
  TDD tasks + `isFacultyCheck` refactor landed + whole-branch review + live verify + Codex spec/plan/impl passes
  all folded; gate green (733/74, lint, validator 8/8, build). **Next:** merge PR #84. Prior branches merged:
  #83 (Phase 2b), #82 (Phase 2a), #76 (Comet Club), #79/#80 (UI/UX Phase 0/1). All audit backlogs cleared except
  #20 (media, user-blocked).
...
**UI/UX track (active) — Phase 4 DONE, PR #85 open.** The last scheduled UI/UX phase (a11y hardening sweep) implemented
on `feat/phase4-a11y-sweep`: 9 tasks + 2 follow-up fixes (the WCAG 2.4.3 focus-restore bug + the Codex-impl test-quality
fold), all per-task spec+quality reviewed, whole-branch integration review (which caught the focus-restore bug), live
in-browser verify, and **file-based Codex spec/plan/impl passes** (4+3 / 6+1 / 3, all folded); gate green (766/78).
**Next: merge PR [#85](https://github.com/JonWhiteFang/gaslight-and-grimoire/pull/85)** (merge commit, never squash).
Phase 3 (#84) merged. **After Phase 4: Phase 5 — choice-gating content model** (`visibility`/`gateReason` schema +
validator rule + authoring-doc update) is the last roadmap item; per the roadmap's live caveat, consider pulling it
forward if content (The Orrery Room) resumes, so new authoring uses the `hidden`/`disabled-with-reason` vocabulary from
day one.
