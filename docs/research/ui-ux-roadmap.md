...
| **2a** ✅ | **Deduction feedback legibility (legibility slice)** — `connected` 🔗 cue + board-owned outcome banner via `announce()` + directional copy. **Formation unchanged; does NOT enact ADR-0012.** | 6, 2 | Top product value; the redundant-cue fix rode along | file-based Codex (spec ×3 rounds → 2a/2b split, plan, impl) — **done: PR #82** |
| **2b** `[~]` | **Deduction formation model** (deferred from 2a) — recipe/`connectsTo` two-graph correctness oracle over player topology; enacts ADR-0012; fixes the pre-existing `contested`-revert bug. **[Spec](../superpowers/specs/2026-07-15-phase2b-deduction-formation-design.md) + [plan](../superpowers/plans/2026-07-16-phase2b-deduction-formation.md) written, both Codex-reviewed (5 findings each, folded in); implementation next.** Origin analysis: [Phase 2 spec Part B](../superpowers/specs/2026-07-14-phase2-deduction-feedback-design.md). | 6 | Split out because three spec-review rounds showed the oracle needs pre-existing board-plumbing changes | file-based Codex — spec ✅ + plan ✅ done; impl pass pending |
| **3** ✅ | **Dice / probability legibility** — DC + diegetic "Prospects" band (Favourable/Uncertain/Forbidding) + advantage shown pre-roll on all three check surfaces; "vs DC N" at-roll; "Assured" for auto-succeed abilities. Single `isFacultyCheck` gate + pure `checkOdds`. | 7 | Informed by Phase 0; broad — applies to **all** faculty checks, not only deduction | file-based Codex (spec 5 / plan 9 / impl 2, all folded) — **done: PR #84** |
| **4** ✅ | **A11y hardening sweep** (batched) — reduced-motion coverage audit, focus-restore + background inertness across *all* overlays, contrast/focus-indicator pass. **Also fixed a real WCAG 2.4.3 bug: the `inert` ancestor blurred the invoker to `<body>` before `useFocusTrap` captured it, so all overlays restored focus to `<body>`; fixed by capturing the invoker at open time (`restoreFocusTo`).** | 3, 4, 5 | All "sweep the UI against a checklist," no design decisions, one coherent PR | file-based Codex (spec 4+3 / plan 6+1 / impl 3, all folded) — **done: PR #85** |
| **5** ✅ | **Choice-gating content model** — `visibility`/`gateReason` schema (unmet-gate case only, default `hidden`), pure `choiceVisibility.ts` resolver, 5 validator errors + 2 always-on warnings, `LockedChoice` rendering, one demo conversion (Comet Club hub) | 8 | Content-pipeline change; the bridge back toward content (see caveat) | file-based Codex (spec 5+2 / plan 1B+4M+1m / impl 1M+2m, all folded) — **done: PR #86** |
