...
| **2b** | **Deduction formation model** (deferred from 2a) — recipe/`connectsTo` two-graph correctness oracle over player topology; enacts ADR-0012; fixes the pre-existing `contested`-revert bug. Full analysis in the [Phase 2 spec Part B](../superpowers/specs/2026-07-14-phase2-deduction-feedback-design.md). | 6 | Split out because three spec-review rounds showed the oracle needs pre-existing board-plumbing changes | file-based Codex (own spec) |
