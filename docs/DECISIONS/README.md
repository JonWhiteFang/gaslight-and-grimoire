...
| [ADR-0010](ADR-0010-codex-adversarial-review-gates.md) | Codex as cross-provider adversarial reviewer — two mandatory gates | Superseded by [ADR-0013](ADR-0013-codex-file-based-review-handoff.md) | 2026-07-11 | Tooling | complements ADR-0004; allowed-content per ADR-0009 | **Superseded 2026-07-14 by ADR-0013** (MCP transport unreliable in-session). Original: every non-trivial task passes two Codex (OpenAI) MCP review gates — plan before any mutation; complete task diff before completion. Preserved for the decision trail; procedure no longer operative. |
...
| [ADR-0012](ADR-0012-deduction-roll-semantics.md) | Deduction roll semantics — correctness gates, the roll flavours | Enacted | 2026-07-16 | UI/UX track (Phase 0) | builds on ADR-0005 | A recipe-matching connected clue-set always forms its deduction and a non-qualifying set never does; the Reason d20 no longer gatekeeps correct reasoning but sets the outcome tier/framing and feeds Phase 2's partial-tier directional feedback. Generic-connection correctness + the tier→feedback map deferred to the Phase 2 spec. **Enacted 2026-07-16 (Phase 2b):** `deductionOracle.classifyBoard` gates formation, `EvidenceBoard` forms all qualifying deductions, `DeductionButton` rolls only. |
| [ADR-0013](ADR-0013-codex-file-based-review-handoff.md) | Codex cross-provider review moves to a file-based three-checkpoint handoff | Enacted | 2026-07-14 | Tooling | supersedes ADR-0010; complements ADR-0004/0009 | Replaces ADR-0010's MCP two-gate model (unreliable in-session Bedrock auth): Codex review is now a **file-based** handoff (`codex/input` → `codex/output`) at **three** checkpoints — spec, plan, and completed implementation (final pass before merge). MCP `codex` tool not used for reviews; server removed at user scope. Prompt+review committed as a trail. |
