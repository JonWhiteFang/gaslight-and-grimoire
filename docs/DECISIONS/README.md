...
| [ADR-0012](ADR-0012-deduction-roll-semantics.md) | Deduction roll semantics — correctness gates, the roll flavours | Accepted | 2026-07-14 | UI/UX track (Phase 0) | builds on ADR-0005 | A recipe-matching connected clue-set always forms its deduction and a non-qualifying set never does; the Reason d20 no longer gatekeeps correct reasoning but sets the outcome tier/framing and feeds Phase 2's partial-tier directional feedback. Generic-connection correctness + the tier→feedback map deferred to the Phase 2 spec. `Accepted`; promote to `Enacted` in the Phase 2 PR that adds the roll-independent-formation test. |
